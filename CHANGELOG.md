...
### 수정
- 새 버전 확인이 한 번도 성공하지 못한 환경(오프라인·비공개·차단망)에서 "하루 1회" 백오프가 동작하지 않아 **모든 명령이 매번 조회를 시도**(명령당 최대 ~6초 지연)하던 문제. 재조회 판정을 마지막 확인 시각만으로 하도록 고침 (M10 W1)
- 문서: non-fast-forward 시 "로컬 커밋은 이미 됐고 푸시만 거부" 임을 명시 (M10 S1)
