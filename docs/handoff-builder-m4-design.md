...
사용자가 새 exe(`v0.3.3`)로 1225 검증(통과) + diff 행 선택·Ctrl+C 복사를 확인하고 **M4 를 승인**했다. 디자인 게이트 종료. 참고: v0.3.2 → v0.3.3 은 exe 안에서 검증이 GUI 자신을 파이썬으로 실행하던 버그 수정(`checker.resolve_python`) — 화면 변경 없음.
