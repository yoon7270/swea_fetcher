...
| M8 | 2026-09-17 | **승인** — 사용자 지적으로 트리거 교체: "로컬 검증 통과" → **"SWEA 제출 Pass"**. `submit.py`(compile.do→submit.do), `service.submit_problem`, CLI `submit [--push]`(exit 8), GUI [SWEA 제출]·최근 메뉴. 실사용에서 버그 2건 잡음: (a) 제출 category 누락 → v0.6.1, (b) 제출이 공개 Problem(CODE) 이 아니라 사용자가 채점받는 모의/클럽 상자(BOX) 로 가야 함 → v0.6.2. 사용자가 1225 제출 → 모의 제출 이력에 기록 확인. 상세: `docs/handoff-planner-m8.md`. **v0.6.2** |
